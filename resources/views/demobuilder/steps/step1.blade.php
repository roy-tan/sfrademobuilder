...
<div class="step">
    ...
    <h3 class="main_question"><strong>1/6</strong>Let's start</h3>
    ...
    <div class="form-group add_top_30">
        <label>  Special Features (Not Active - just testing)
        <span  class="tooltip-icon" data-toggle="tooltip" data-placement="right" title="Please select special features you may want">?</span>
        </label>
        <select class="form-control required" name="special_features">
            @foreach(config('demo.special_features') as $specialfeature)
            <option value="{{$specialfeature}}">{{$specialfeature}}</option>
            @endforeach
        </select>
        </div>
