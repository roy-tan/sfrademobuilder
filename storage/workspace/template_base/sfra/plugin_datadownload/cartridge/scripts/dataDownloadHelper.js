 ...
 * profile, product lists, orders, wallet, addresses, and a place holder for third party
 * information.
 *
 */
 ...
 * @returns {Object} an object containing user profile information
 */
...
/**
 * get stored user addresses
 * @param {Object} profile - current customer's profile
 * @returns {Array<Object>} an array of stored addresses
 */
...
/**
 * get stored user payment instruments
 * @param {Object} profile - current customer's profile
 * @returns {Array<Object>} an array of stored payment instruments
 */
 ...
 * get user orders
 * @param {Object} profile - current customer's profile
 * @returns {Array<Object>} an array of objects containing order information
 */
function getOrders(profile) {
    ...
    while (orders.hasNext()) {
        ...
        var orderObj = {
            ...
            replacedOrder: order.replacedOrder
                ? {
                    ...
                    refundedAmount: order.replacedOrder.refundedAmount.currencyCode
                    + ' '
                    + order.replacedOrder.refundedAmount.value,
                    ...
                    totalGrossPrice: order.replacedOrder.totalGrossPrice.currencyCode
                    + ' '
                    + order.replacedOrder.totalGrossPrice.value
                }
                : {},
        ...
        orderPaymentInstruments.toArray().forEach(
            function (paymentInstrument) { // eslint-disable-line no-loop-func
                orderObj.paymentInstruments.push({
                    capturedAmount: paymentInstrument.capturedAmount.currencyCode
                    + ' '
                    + paymentInstrument.capturedAmount.value,
                    creditCardExpirationMonth: paymentInstrument.creditCardExpirationMonth,
                    creditCardExpirationYear: paymentInstrument.creditCardExpirationYear,
...
/**
 * get stored user wish lists
 * @param {dw.customer.Customer} customer - Name of the route to replace
 * @param {dw/customer/ProductListMgr} ProductListMgr - productListMgr
 * @returns {Array<Object>} an array of stored wish lists
 */
function getWishLists(customer, ProductListMgr) {
    var ProductList = require('dw/customer/ProductList');

    var wishListsObj = [];
    var wishlists = ProductListMgr.getProductLists(
        customer,
    ...
    wishlists.forEach(function (wishlist) {
        wishListsObj.push({
            ...
            shippingAddress: {
                ...
                countryCode: wishlist.shippingAddress
                    ? wishlist.shippingAddress.countryCode.displayValue
                    : null,
...
/**
 * get stored user gift registries
 * @param {dw.customer.Customer} customer - Name of the route to replace
 * @param {dw/customer/ProductListMgr} ProductListMgr - productListMgr
 * @returns {Array<Object>} an array of stored gift registries
 */
function getGiftregistries(customer, ProductListMgr) {
    var ProductList = require('dw/customer/ProductList');

    var giftregistriesObj = [];
    var giftregistries = ProductListMgr.getProductLists(
        customer,
    ...
    giftregistries.forEach(function (giftregistry) {
        giftregistriesObj.push({
            ...
            postEventShippingAddress: giftregistry.postEventShippingAddress
                ? {
            ...
            shippingAddress: giftregistry.shippingAddress
                ? {
...
/**
 * get stored user shopping lists
 * @param {dw.customer.Customer} customer - Name of the route to replace
 * @param {dw/customer/ProductListMgr} ProductListMgr - productListMgr
 * @returns {Array<Object>} an array of stored shopping lists
 */
function getShoppingLists(customer, ProductListMgr) {
    var ProductList = require('dw/customer/ProductList');

    var shoppingListObj = [];
    var shoppinglists = ProductListMgr.getProductLists(
        customer,
    ...
    shoppinglists.forEach(function (shoppinglist) {
        shoppingListObj.push({
            ...
            shippingAddress: shoppinglist.shippingAddress
                ? {
...
exports.getProfileData = function (customer) {
    var ProductListMgr = require('dw/customer/ProductListMgr');
    var downloadJSONObj = {};

    downloadJSONObj.profile = getProfile(customer.profile);
    downloadJSONObj.addressbook = getAddressBook(customer.profile);
    downloadJSONObj.wallet = getWallet(customer.profile);
    downloadJSONObj.orders = getOrders(customer.profile);
    downloadJSONObj.productList = {
        wishlists: getWishLists(customer, ProductListMgr),
        giftregistries: getGiftregistries(customer, ProductListMgr),
        shoppinglists: getShoppingLists(customer, ProductListMgr)
    };
